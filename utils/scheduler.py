...
class FlowMatchScheduler():
    ...
    def training_weight_SelfFlow(self, timestep):
        """
        Per-token training weight for SelfFlow.
        Unlike training_weight() which expects [B*T] timestep, this accepts
        arbitrary-shaped timestep tensors (e.g. [B, F, 1, H, W]).
        Input:
            - timestep: any shape (e.g. [B, F, 1, H, W])
        Output: weights with same shape as timestep
        """
        ...
        weights = self.linear_timesteps_weights[timestep_id].reshape(orig_shape)
        return weights
